...
Escreva apenas o parágrafo literário de transição (aterrissagem de consciência), em português.`,
    ...
    if (modoByok) {
      const jsonTurno = JSON.stringify({
        ...
        ritmo_da_cena_diretor_de_cinema: ritmoAtual,
...
No campo "prosa", escreva a continuação literária direta, em português. Sem introduções, sem avisos sistêmicos. Apenas a pura narrativa. Em "memorias_registradas", gere a memória subjetiva do evento para CADA personagem presente ou afetado na cena (cada um só percebe o que viveu — perspectivas isoladas). Em "memorias_evocadas": se a cena natural e organicamente faz um personagem reviver uma lembrança (gatilho sensorial, objeto, nome, lugar), retorne essa lembrança em primeira pessoa dele — será renderizada como um flashback destacado; escreva-a como prosa literária curta, não force, só evoque quando o gatilho existir de fato no texto. Se a lembrança evocar uma pessoa que NÃO está entre os personagens já cadastrados e a história plausivelmente permite que ela reapareça (viva, relevante, apenas perdida de vista), preencha "personagem_evocado" com pode_reaparecer=true (o sistema forjará a personalidade dela coerente com a lembrança); se está morta e não voltará, pode_reaparecer=false. Em "objetos_manipulados": se um objeto for usado, alterado, movido, quebrado ou trocar de dono neste turno, ou se um objeto inédito com peso narrativo surgir, registre-o (name, novo_estado e, quando aplicável, nova_localizacao / novo_dono_character_name / acao). Só objetos com peso narrativo — não catalogue poeira. Em "personagens_em_cena", liste TODOS os personagens que continuam fisicamente presentes neste local após o turno — mesmo os que você não citou diretamente na prosa (não mencionar alguém não significa que ele saiu). NÃO inclua alguém aqui só porque foi mencionado, lembrado ou citado de passagem no texto — só inclua quem está FISICAMENTE no local. Em "personagens_que_saem_de_cena", liste APENAS quem a prosa deste turno mostra explicitamente saindo, partindo ou sendo removido do local (ex: "Fulano sai correndo", morte, teletransporte). Nunca remova alguém de cena por omissão — só por saída narrada.`,
      response_json_schema: {
        ...
        properties: {
          ...
          personagens_em_cena: { type: 'array', items: { type: 'string' }, description: 'TODOS os personagens que continuam fisicamente presentes, mesmo os não citados diretamente na prosa' },
          personagens_que_saem_de_cena: { type: 'array', items: { type: 'string' }, description: 'Apenas quem a prosa mostra explicitamente saindo/partindo/sendo removido neste turno' },
    ...
    // Nunca confia cegamente na lista do Diretor como a cena inteira — quem ele não citou não é quem saiu.
    const saidasExplicitas = new Set(resultado.personagens_que_saem_de_cena || []);
    const cenaUniao = new Set([...(story.characters_in_scene || []), ...(resultado.personagens_em_cena || []), ...entrantesMemoria]);
    const cenaFinal = [...cenaUniao].filter((nome) => !saidasExplicitas.has(nome));
    await sdk.entities.Story.update(story.id, {
