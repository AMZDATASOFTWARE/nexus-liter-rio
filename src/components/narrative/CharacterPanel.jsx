...
import CharacterAssetsDialog from "./CharacterAssetsDialog";

export default function CharacterPanel({ characters, povName, inScene = [], universeId }) {
  const [personagemAssets, setPersonagemAssets] = useState(null);
  ...
  return (
    <div className="space-y-3">
          ...
          <div key={c.id} className={`rounded-xl border p-3.5 transition-colors ${isPov ? "border-amber-400/30 bg-amber-400/5" : "border-zinc-800 bg-zinc-900/50"}`}>
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                {universeId && (
                  <button
                    onClick={() => setPersonagemAssets(c.name)}
                    className="text-zinc-600 hover:text-amber-300 transition-colors"
                    title="Referências de aparência (imagem/PDF)"
                  >
      ...
      {universeId && (
        <CharacterAssetsDialog
          open={!!personagemAssets}
          onOpenChange={(open) => !open && setPersonagemAssets(null)}
          universeId={universeId}
          characterName={personagemAssets}
        />
      )}
    </div>
  );
